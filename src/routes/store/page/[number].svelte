<script context="module">
  ...
  export async function preload({ params, query }) {
    const res = await this.fetch(
      `${apiUrl}${params.number !== 1 ? `/page/${params.number}` : ""}/index.json`
    );
    const posts = await res.json();

    return { posts, currentPage: +params.number };
  }
</script>
...
<div class="flex flex-col lg:flex-row justify-between items-center">
  <div class="text-xs text-gray-500 items-center mb-4 lg:mb-0">Page {currentPage} of {count}</div>
  <div class="flex-1 flex items-center text-center justify-center">
    ...
    {#if currentPage > 2}
      ...
      <li class="pagination__item list-none mx-1"><a rel="prefetch" href="{currentPage !== 1 ? `store/page/${currentPage - 1}` : 'store' }" class="text-gray-600 hover:text-secondary hover:no-underline bg-white rounded-sm shadow hover:shadow-lg py-1 px-3">«</a></li>
    {/if}
    {#each { length: pages } as _, i}
    <li class="pagination__item list-none mx-1"><a rel="prefetch" href="{(i + 1) !== 1 ? `store/page/${i + 1}` : 'store' }" class="pagination__item-link text-gray-600 hover:text-secondary hover:no-underline bg-white rounded-sm shadow hover:shadow-lg py-1 px-3 { (i + 1) === currentPage ? 'active' : '' }">{i + 1}</a></li>
    {/each}
